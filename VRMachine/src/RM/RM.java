...
public class RM {
	...
	/**
	 * Atminties vietai adresu XX nustatoma nauja reikÃ°mÃ«
	 * 
	 * @param xx
	 *            Atminties adresas
	 * @param R
	...
	public static void Interrupt() {
		if (PI.get() != 0) {
			switch (PI.get()) {
			case 1: {
				MainWindow.updateConsole("Bandoma naudoti neteisingÄ… adresÄ…");
				break;
			}
			case 2: {
			...
			default: {
				System.out.println("Pertraukimas PI neÄ¯vyko.");
				break;
			}
			}
			IP.set(IP.get()+1);
		...
		if (SI.get() != 0) {
			...
			default: {
				MainWindow.updateConsole("Pertraukimas SI neÄ¯vyko.");
				break;
			}
			}
			IP.set(IP.get()+1);
		...
		if (TIMER.get() != 0) {
			...
			default: {
				MainWindow.updateConsole("Pertraukimas T neÄ¯vyko.");
			}
			}
			IP.set(IP.get()+1);
			//MODE.set(0);
	...
	private void step() {
		...
		if(SI.get() != 3) {
			...
			updateGUI();
			MainWindow.updateConsole(">> Programa baigÄ— darbÄ…!");
			MODE.set(1);
		}
	}
